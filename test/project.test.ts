...
describe("Integration tests", function () {
  describe("Hardhat Runtime Environment extension", function () {
    ...
    const testABI = "function deploy(bytes _initCode, bytes32 _salt) returns (address createdContract) @29000000";
    const eoaAddress = "0x000000000000000000000000000000000000dEaD";

    it("should get the Contract ABI from Etherscan", async function () {
      // @ts-ignore
      const contract = await this.env.getVerifiedContractAt(testAddress);
    ...
    it("should detect bad address", async function () {
      // @ts-ignore
      await expect(this.env.getVerifiedContractAt("foo")).to.be.rejectedWith(
    ...
    it("should correctly fail on unverified contracts", async function () {
      ...
      // @ts-ignore
      await expect(this.env.getVerifiedContractAt(eoaAddress)).to.be.rejectedWith(
